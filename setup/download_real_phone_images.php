<?php
/**
 * Download Real Phone Images Script
 * Downloads actual phone images from free sources
 */
...
echo "<h1>📱 Downloading Real Phone Images</h1>";

// Real phone image URLs from free sources
$phoneImages = [
    'iPhone 15 Pro Max' => [
        'https://images.unsplash.com/photo-1592750475338-74b7b21085ab?w=400&h=600&fit=crop&q=80',
        'https://images.unsplash.com/photo-1511707171634-5f897ff02aa9?w=400&h=600&fit=crop&q=80',
        'https://images.unsplash.com/photo-1510557880182-3d4d3cba35a5?w=400&h=600&fit=crop&q=80'
    ],
...
foreach ($phoneImages as $phoneName => $urls) {
    ...
    foreach ($urls as $index => $url) {
        ...
        if (downloadImage($url, $filepath)) {
            echo "✓ Success<br>";
            $phoneImagePaths[] = 'uploads/phones/' . $filename;
            $successCount++;
        } else {
            echo "✗ Failed<br>";
        }
    ...
    if (!empty($phoneImagePaths)) {
        $downloadedImages[$phoneName] = implode(',', $phoneImagePaths);
    }
}
...
if ($successCount > 0) {
    ...
    file_put_contents($phonesDir . '/phone_image_mapping.json', json_encode($downloadedImages, JSON_PRETTY_PRINT));
